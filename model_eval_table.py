...
with open(csv_path, mode="a", newline="") as f:
    ...
    for ModelClass in all_model_classes:
        ...
        # Before training, check if the model needs flattened input
        if model_name == "Lightweight_DNN_CIR":
            # Flatten last two dimensions: (612, 14) → (8568,)
            def flatten_map(x, y):
                return tf.reshape(x, [-1, 612 * 14]), y

            # train_dataset = train_dataset.map(flatten_map)
            # val_dataset = val_dataset.map(flatten_map)
            val_data = val_data.reshape(-1, 612 * 14)  # Reshape to (N, 8568)
            val_labels = val_labels.reshape(-1, 612 * 14)  # Reshape to (N, 8568)
        ...
        # Compute MSE with correct axis depending on model
        if model_name == "Lightweight_DNN_CIR":
            predictions = predictions.reshape(-1, 612 * 14)  # flatten predictions
